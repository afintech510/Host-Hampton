...
import { ArrowLeft, ArrowRight, Calendar, Users, MapPin, Clock, Palette, Gift, Star, Crown } from "lucide-react";
...
const PARTY_THEMES = [
  ...
  "Mermaid", "Football", "Basketball", "Soccer", "Art & Craft", 
  "Science", "Cooking", "Dance", "Music", "Custom"
];

const PARTY_ADDONS = [
  "Goodie Bags", "Balloon Tower", "Photo Booth", "Extra Activity", 
  "Fancy Desserts", "Face Painting", "Glitter Tattoos", "Hair Tinsel"
];

const FOOD_OPTIONS = [
  "Pizza Party Package", "Sandwich Platters", "Fruit & Veggie Tray",
  "Juice Boxes", "Water Bottles", "Special Dietary Options"
];

const JEWELRY_PIECES = [
  "Bracelet", "Anklet", "Necklace", "Ring", "Earrings"
];
...
export default function Quote() {
  ...
  const getTotalSteps = () => {
    switch (formData.serviceType) {
      case 'kids-party': return 7; // service -> theme -> package -> addons -> date/child -> food -> contact
      case 'studio-rental': return 5; // service -> subtype -> group -> datetime -> contact
      case 'trucker-hat': return 6; // service -> count/age -> color/patch -> datetime -> location -> contact
      case 'workshop': return 4; // service -> format/type -> attendees/schedule -> contact
      case 'jewelry': return 4; // service -> pieces/count -> datetime -> contact
      case 'general': return 2; // service -> contact
      default: return 2;
    }
  };
    ...
    <Card className="w-full max-w-2xl mx-auto">
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            { id: 'kids-party', title: 'Kids Themed Party', icon: Gift, desc: 'Birthday parties with themes, activities & more' },
            { id: 'studio-rental', title: 'Studio Rental', icon: MapPin, desc: 'Private space for your event or photoshoot' },
            { id: 'trucker-hat', title: 'Trucker Hat Bar', icon: Crown, desc: 'Custom trucker hat making experience' },
            { id: 'workshop', title: 'Workshop / Class', icon: Star, desc: 'Educational and creative workshops' },
            { id: 'jewelry', title: 'Permanent Jewelry', icon: Palette, desc: 'Custom permanent jewelry appointments' },
            { id: 'general', title: 'General Inquiry', icon: Users, desc: 'Other questions or custom requests' }
          ].map(service => {
              ...
              >
                <Card 
                  className={`cursor-pointer transition-all border-2 hover:border-purple-300 ${
                    formData.serviceType === service.id ? 'border-purple-500 bg-purple-50' : 'border-gray-200'
                  }`}
  ...
  const renderContactStep = () => (
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        ...
        <p className="text-gray-600">We'll use this information to send you a custom quote</p>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="flex items-start space-x-2">
          ...
          <Label htmlFor="consent" className="text-sm leading-relaxed">
            I consent to Host Hampton contacting me via email, phone, or text regarding this booking and future events. 
            This helps us provide you with updates about your event and information about our services. *
          </Label>
        </div>
        ...
        <div className="flex justify-between pt-6">
          ...
          <Button 
            ...
            disabled={!formData.firstName || !formData.lastName || !formData.email || !formData.phone || !formData.consent || submitQuoteMutation.isPending}
            className="bg-purple-600 hover:bg-purple-700"
          >
          ...
          <Card className="w-full max-w-2xl mx-auto">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-between pt-6">
                ...
                <Button 
                  onClick={handleNext}
                  disabled={!formData.partyTheme || (formData.partyTheme === 'Custom' && !formData.customTheme)}
                  className="bg-purple-600 hover:bg-purple-700"
                >
                  Continue <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
        ...
        return (
          <Card className="w-full max-w-2xl mx-auto">
            <CardHeader>
              ...
              <p className="text-gray-600">Base package starts at $875 for 11 attendees (including birthday child)</p>
            </CardHeader>
            <CardContent className="space-y-4">
              <RadioGroup 
                value={formData.partyPackage} 
                onValueChange={(value) => updateFormData({ partyPackage: value })}
              >
                <div className="space-y-4">
                  <div className="border rounded-lg p-4">
                    <div className="flex items-center space-x-2 mb-2">
                      <RadioGroupItem value="base" id="base" />
                      <Label htmlFor="base" className="text-lg font-semibold">Base Package</Label>
                    </div>
                    <p className="text-gray-600 ml-6">Essential party experience with theme activities and supplies</p>
                  </div>
                  
                  <div className="border rounded-lg p-4">
                    <div className="flex items-center space-x-2 mb-2">
                      <RadioGroupItem value="make-it-shine" id="make-it-shine" />
                      <Label htmlFor="make-it-shine" className="text-lg font-semibold">Make it Shine</Label>
                      <span className="text-purple-600 font-medium">+$25 per attendee</span>
                    </div>
                    <p className="text-gray-600 ml-6">Enhanced experience with premium add-ons and decorations</p>
                  </div>
                  
                  <div className="border rounded-lg p-4">
                    <div className="flex items-center space-x-2 mb-2">
                      <RadioGroupItem value="party-envy" id="party-envy" />
                      <Label htmlFor="party-envy" className="text-lg font-semibold">Party Envy</Label>
                      <span className="text-purple-600 font-medium">+$50 per attendee</span>
                    </div>
                    <p className="text-gray-600 ml-6">Ultimate party experience with all premium features and extras</p>
                  </div>
                </div>
              </RadioGroup>
        ...
        return (
          <Card className="w-full max-w-2xl mx-auto">
            <CardHeader>
              ...
              <p className="text-gray-600">Choose additional services to enhance your party</p>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                  ...
                  <div key={addon} className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => {
                        ...
                        if (checked) {
                          updateFormData({ partyAddons: [...currentAddons, addon] });
                        } else {
                          updateFormData({ partyAddons: currentAddons.filter(a => a !== addon) });
                        }
                      }}
                    />
        ...
        return (
          <Card className="w-full max-w-2xl mx-auto">
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <Label htmlFor="attendeeCount">Number of Attendees (including birthday child)</Label>
                <Input
                  id="attendeeCount"
                  type="number"
                  min="1"
          ...
          <Card className="w-full max-w-2xl mx-auto">
            ...
            <CardContent className="space-y-4">
                    ...
                    <div key={food} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          ...
                          if (checked) {
                            updateFormData({ foodOptions: [...currentFood, food] });
                          } else {
                            updateFormData({ foodOptions: currentFood.filter(f => f !== food) });
                          }
                        }}
                      />
              ...
              <div>
                ...
                <Select value={formData.cupcakeOptions} onValueChange={(value) => updateFormData({ cupcakeOptions: value })}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select cupcake option" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="host-hampton-cupcakes">Host Hampton Custom Cupcakes</SelectItem>
                    <SelectItem value="bring-own">I'll bring my own cupcakes</SelectItem>
                    <SelectItem value="no-cupcakes">No cupcakes needed</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              
              <div>
                <Label htmlFor="specialNeeds">Special Dietary Needs or Allergies</Label>
                <Textarea
                  id="specialNeeds"
          ...
          <Card className="w-full max-w-2xl mx-auto">
            ...
            <CardContent className="space-y-4">
              <RadioGroup 
                ...
                onValueChange={(value) => updateFormData({ studioSubType: value })}
              >
              ...
              {formData.studioSubType === 'other' && (
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateFormData({ customStudioType: e.target.value })}
              ...
              <div>
                ...
                <Textarea
                  ...
                  onChange={(e) => updateFormData({ studioDescription: e.target.value })}
              ...
              <div className="flex justify-between pt-6">
                ...
                <Button 
                  onClick={handleNext}
                  disabled={!formData.studioSubType || (formData.studioSubType === 'other' && !formData.customStudioType)}
                  className="bg-purple-600 hover:bg-purple-700"
                >
                  Continue <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => updateFormData({ studioAttendeeCount: e.target.value })}
              ...
              <div>
                ...
                <RadioGroup 
                  ...
                  onValueChange={(value) => updateFormData({ studioGroupType: value })}
                >
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center space-x-2 mb-4">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => updateFormData({ studioDateFlexible: checked as boolean })}
                />
                <Label htmlFor="studioDateFlexible">Not sure yet / Flexible with dates</Label>
              </div>
                ...
                <>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => updateFormData({ studioDate: e.target.value })}
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updateFormData({ studioTime: e.target.value })}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updateFormData({ studioEndTime: e.target.value })}
                      />
                    </div>
                  </div>
                </>
              ...
              <div>
                ...
                <Textarea
                  ...
                  onChange={(e) => updateFormData({ studioTimeNotes: e.target.value })}
      ...
      case 1: // Attendee Count & Age Range
        return (
          <Card className="w-full max-w-2xl mx-auto">
            ...
            <CardContent className="space-y-4">
              <div>
                <Label htmlFor="truckerAttendeeCount">How many people will be making hats?</Label>
                <Input
                  id="truckerAttendeeCount"
                  type="number"
                  min="1"
                  value={formData.truckerAttendeeCount || ''}
                  onChange={(e) => updateFormData({ truckerAttendeeCount: e.target.value })}
              ...
              <div>
                ...
                <RadioGroup 
                  ...
                  onValueChange={(value) => updateFormData({ truckerAgeRange: value })}
                >
              ...
              <div className="flex justify-between pt-6">
                ...
                <Button 
                  onClick={handleNext}
                  disabled={!formData.truckerAttendeeCount || !formData.truckerAgeRange}
                  className="bg-purple-600 hover:bg-purple-700"
                >
                  Continue <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
      ...
      case 2: // Color & Patch Theme
        return (
          <Card className="w-full max-w-2xl mx-auto">
            ...
            <CardContent className="space-y-4">
              <div>
                <Label htmlFor="truckerColorTheme">Color Theme / Preferences</Label>
                <Input
                  id="truckerColorTheme"
                  value={formData.truckerColorTheme || ''}
                  onChange={(e) => updateFormData({ truckerColorTheme: e.target.value })}
                  placeholder="e.g., Pink & Gold, Team Colors, Rainbow, etc."
                />
              </div>
              
              <div>
                <Label htmlFor="truckerPatchTheme">Patch Theme / Design Ideas</Label>
                <Textarea
                  id="truckerPatchTheme"
                  value={formData.truckerPatchTheme || ''}
                  onChange={(e) => updateFormData({ truckerPatchTheme: e.target.value })}
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center space-x-2 mb-4">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => updateFormData({ truckerDateFlexible: checked as boolean })}
                />
                <Label htmlFor="truckerDateFlexible">Not sure yet / Flexible with dates</Label>
              </div>
                ...
                <>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => updateFormData({ truckerDate: e.target.value })}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => updateFormData({ truckerTime: e.target.value })}
                    />
                  </div>
                </>
              )}
          ...
          <Card className="w-full max-w-2xl mx-auto">
            ...
            <CardContent className="space-y-4">
              <RadioGroup 
                ...
                onValueChange={(value) => updateFormData({ truckerLocation: value })}
              >
                <div className="space-y-3">
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="host-hampton" id="host-hampton" />
                    <Label htmlFor="host-hampton">At Host Hampton Studio</Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="mobile" id="mobile" />
                    <Label htmlFor="mobile">Mobile Service (We come to you)</Label>
                  </div>
                </div>
              </RadioGroup>
              ...
              {formData.truckerLocation === 'mobile' && (
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateFormData({ truckerMobileLocation: e.target.value })}
              ...
              <div className="flex justify-between pt-6">
                ...
                <Button 
                  onClick={handleNext}
                  disabled={!formData.truckerLocation || (formData.truckerLocation === 'mobile' && !formData.truckerMobileLocation)}
                  className="bg-purple-600 hover:bg-purple-700"
                >
                  Continue <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <RadioGroup 
                  ...
                  onValueChange={(value) => updateFormData({ workshopFormat: value })}
                >
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="in-person" id="in-person" />
                      <Label htmlFor="in-person">In-Person at Host Hampton</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="mobile" id="mobile-workshop" />
                      <Label htmlFor="mobile-workshop">Mobile (We come to you)</Label>
                    </div>
                    <div className="flex items-center space-x-2">
              ...
              <div>
                <Label htmlFor="workshopType">What type of workshop/class?</Label>
                <Textarea
                  id="workshopType"
                  value={formData.workshopType || ''}
                  onChange={(e) => updateFormData({ workshopType: e.target.value })}
      ...
      case 2: // Attendees & Schedule
        return (
          <Card className="w-full max-w-2xl mx-auto">
            ...
            <CardContent className="space-y-4">
              <div>
                <Label htmlFor="workshopAttendeeCount">How many attendees?</Label>
                <Input
                  id="workshopAttendeeCount"
                  type="number"
                  min="1"
                  value={formData.workshopAttendeeCount || ''}
                  onChange={(e) => updateFormData({ workshopAttendeeCount: e.target.value })}
                  placeholder="Number of participants"
                />
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => updateFormData({ workshopDate: e.target.value })}
                />
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => updateFormData({ workshopTime: e.target.value })}
                />
              </div>
              ...
              <div>
                ...
                <Textarea
                  ...
                  onChange={(e) => updateFormData({ workshopNotes: e.target.value })}
      ...
      case 1: // Jewelry Pieces & People Count
        return (
          <Card className="w-full max-w-2xl mx-auto">
            ...
            <CardContent className="space-y-4">
              <div>
                <Label className="text-base font-medium">What jewelry pieces are you interested in?</Label>
                <div className="grid grid-cols-2 gap-3 mt-2">
                  {JEWELRY_PIECES.map(piece => (
                    <div key={piece} className="flex items-center space-x-2">
                      <Checkbox
                        id={piece}
                        checked={formData.jewelryPieces?.includes(piece) || false}
                        onCheckedChange={(checked) => {
                          const currentPieces = formData.jewelryPieces || [];
                          if (checked) {
                            updateFormData({ jewelryPieces: [...currentPieces, piece] });
                          } else {
                            updateFormData({ jewelryPieces: currentPieces.filter(p => p !== piece) });
                          }
                        }}
                      />
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => updateFormData({ jewelryAttendeeCount: e.target.value })}
              ...
              <div className="flex justify-between pt-6">
                ...
                <Button 
                  onClick={handleNext}
                  disabled={!formData.jewelryPieces?.length || !formData.jewelryAttendeeCount}
                  className="bg-purple-600 hover:bg-purple-700"
                >
                  Continue <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => updateFormData({ jewelryDate: e.target.value })}
                />
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => updateFormData({ jewelryTime: e.target.value })}
                />
              </div>
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Tell us about your event and we'll create a custom quote for you</p>
        </div>
