...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Party Quote submission endpoint
  app.post("/api/party-quotes", async (req, res) => {
    try {
      const quoteData = req.body;
      console.log("Party quote data received:", JSON.stringify(quoteData, null, 2));
      
      // For now, create a simple lead record to track the quote
      // Later we'll enhance this to create proper quote/invoice records
      const leadData = {
        source: "party-quote",
        eventType: "birthday-party", 
        name: "Party Quote", // Will be updated when contact info is added
        email: null, // Will be updated when contact info is added
        phone: null,
        eventDate: null,
        guestCount: quoteData.guestCount || 10,
        status: "draft",
        leadScore: "warm",
        formStep: "theme-package",
        formData: quoteData, // Store the entire form data for later processing
        // Include the quote ID for permalink access
        customTheme: quoteData.quoteId ? `quote-${quoteData.quoteId}` : null
      };
      ...
      res.json({ 
        success: true, 
        message: "Party quote created successfully",
        quoteId: quoteData.quoteId,
        leadId: lead.id 
      });
    } catch (error: any) {
      console.error("Error creating party quote:", error);
      res.status(500).json({ 
        success: false, 
        message: "Failed to create party quote" 
      });
    }
  });

  // Get party quote by ID (for permalink access)
  app.get("/api/party-quotes/:quoteId", async (req, res) => {
    try {
      const { quoteId } = req.params;
      
      // Find lead by quoteId (stored in customTheme field for now)
      const leads = await storage.getLeads();
      const lead = leads.find(l => l.customTheme === `quote-${quoteId}`);
      ...
      console.error("Error fetching party quote:", error);
      res.status(500).json({ 
        success: false, 
        message: "Failed to fetch party quote" 
      });
    }
  });
